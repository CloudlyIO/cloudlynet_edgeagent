...
- GenieACS device IDs are opaque across inventory, heartbeat, telemetry, and snapshots. If an ID contains a literal percent sequence such as `%2D`, `SendSnapshot` path-escapes the whole identifier (`%252D` on the wire) so the HTTP router decodes it exactly once and resolves the same cloud NanoLink row.
